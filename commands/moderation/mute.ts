...
module.exports = {
    ...
    callback: async (client: Client, bot: any, message: Message, args: string[]) => {
        try {
            ...
            if (muteUser.id === message.guild.ownerId) { return message.channel.send({ content: "Unable to issue punishments to this user!" }) }
            ...
            if (!/^\d/.test(args[1])) {
                ...
                newCases.save().catch((err: Error) => ErrorLog(message.guild!, "NEW_CASE", err, client, message, `${message.author.id}`, `mute.ts`))
            ...
            (err: Error) => {
                ErrorLog(message.guild!, "MUTE_COMMAND", err, client, message, `${message.author.id}`, `mute.ts`)
            }
        }
    },
}