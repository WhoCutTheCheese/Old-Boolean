        ...
        try {
            if (message) {
                ...
                interaction.guild?.members.fetch().then((fetchedMembers: any) => {
                    ...
                    const botCount = interaction.guild?.members.cache.filter((member: any) => !member.user.bot).size;
                    const botCount2 = interaction.guild?.members.cache.filter((member: any) => member.user.bot).size;
                    const serverinfo = new MessageEmbed()
