...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("modrole")
        .setDescription("Add a mod role. Mod roles allow users to use moderation commands.")
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client) {
        ...
        switch (subCommand) {
            case "add":
                ...
                if (configuration?.modRoleID.includes(role.id)) return interaction.reply({ content: "This role is already defined as a mod role!", ephemeral: true })

                if (configuration?.modRoleID.length! > 5) return interaction.reply({ content: "You've reached the maximum allowed mod roles!", ephemeral: true })
                ...
                const embed = new EmbedBuilder()
                    .setColor(color)
                    .setDescription(`<:yes:979193272612298814> You've successfully added <@&${role.id}> as a mod role!`)
            ...
            case "remove":
                ...
                if (!configuration?.modRoleID.includes(role.id)) return interaction.reply({ content: "This role is not defined as a mod role.", ephemeral: true })

                const roles = configuration.modRoleID
                ...
                const removed = new EmbedBuilder()
                    .setColor(color)
                    .setDescription(`<:no:979193272784265217> You've successfully removed <@&${role.id}> as a mod role!`)
            ...
            case "reset":
                ...
                const confirm = new EmbedBuilder()
                    .setTitle("Reset All Mod Roles?")
                    .setColor(color)
                    .setDescription("This will delete all mod roles from Boolean's database.")
                    .setTimestamp()
