import { ActionRowBuilder, APIButtonComponent, ButtonBuilder, ButtonInteraction, ButtonStyle, Client, ColorResolvable, EmbedBuilder, GuildMember, Message, User } from "discord.js";
...
module.exports = {
    ...
    callback: async (client: Client, message: Message, args: string[]) => {
        ...
        const updatesReply = await message.channel.send({ embeds: [updateEmbed], components: [row] })
            ...
            switch (buttonInteraction.customId) {
                case `left.${buttonInteraction.user.id}`:
                    ...
                    const updatesEmbed = new EmbedBuilder()
                        ...
                        .setColor("Blurple")
                        .setDescription(`\`Latest Version: v${bot.version}\`${updatesArray[numbers]}`)
                    updatesReply.edit({ embeds: [updatesEmbed], components: [row] })
                    break;
                case `end_interaction.${buttonInteraction.user.id}`:
                    ...
                    updatesReply.edit({ embeds: [endedInteractioin], components: [row] })
                    break;
                case `right.${buttonInteraction.user.id}`:
                    ...
                    const historyEmbed2 = new EmbedBuilder()
                        ...
                        .setColor("Blurple")
                        .setDescription(`\`Latest Version: v${bot.version}\`${updatesArray[numbers]}`)
                    updatesReply.edit({ embeds: [historyEmbed2], components: [row] })
                    break;
                default:
