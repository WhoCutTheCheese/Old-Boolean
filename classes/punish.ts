...
export class Punishment {
    ...
    constructor(someArg: {
        ...
        if (type != 4) {
            ...
            if (exists == true) {
                if (message.guild?.members.me?.permissionsIn(modLogChannel!).has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.EmbedLinks, PermissionsBitField.Flags.ViewChannel])) {
        ...
        switch (type) {
            ...
            case 4:
                ...
                if (remainder == 0) {
                    ...
                    if (exists == true) {
                        if (message.guild?.members.me?.permissionsIn(channel!).has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.EmbedLinks, PermissionsBitField.Flags.ViewChannel])) {
                    ...
                    member.timeout(ms("10m")).catch((err: Error) => {
                        console.error(err)
                        message.channel.send({ content: "I could not mute this user!" })
                    });
                    return;
